...
public class MaterialService {
    ...
    public List<Material> getMaterialesByArea(String area) {
        return repository.findByArea(area);
    }

    public List<Material> getMaterialesByNucleo(String nucleo) {
        return repository.findByNucleo(nucleo);
    }

    public List<Material> getMaterialesByAmbito(String ambito) {
        return repository.findByAmbito(ambito);
    }

    public List<Material> getMaterialesByEsDescargable(boolean esDescargable) {
        return repository.findByEsDescargable(esDescargable);
    }

    public List<Material> getMaterialesByEsEditable(boolean esEditable) {
        return repository.findByEsEditable(esEditable);
    }

    public List<Material> getMaterialesByIncluyeSolucionario(boolean incluyeSolucionario) {
        return repository.findByIncluyeSolucionario(incluyeSolucionario);
    }
    ...
    public Material updateMaterial(int id, Material materialDetails, MultipartFile file) {
        return repository.findById(id).map(existingMaterial -> {
            try {
                ...
                existingMaterial.setArea(materialDetails.getArea());
                existingMaterial.setNucleo(materialDetails.getNucleo());
                existingMaterial.setEje(materialDetails.getEje());
                existingMaterial.setAmbito(materialDetails.getAmbito());
                existingMaterial.setObjetivoAprendizaje(materialDetails.getObjetivoAprendizaje());
                existingMaterial.setDescripcion(materialDetails.getDescripcion());
                existingMaterial.setEsOnline(materialDetails.isEsOnline());
                existingMaterial.setEsDescargable(materialDetails.isEsDescargable());
                existingMaterial.setEsEditable(materialDetails.isEsEditable());
                existingMaterial.setIncluyeSolucionario(materialDetails.isIncluyeSolucionario());
